...
uint8_t ui_lv_get_bpm(void) {
    uint8_t wpm = 0;
    wpm = get_current_wpm();
    return wpm;
}

lv_color_hsv_t ui_lv_rgb_matrix_get_hsv(void) {
...
void ui_lv_rgb_matrix_sethsv(uint16_t hue, uint8_t sat, uint8_t val, bool eeprom) {
...
uint8_t ui_lv_rgb_matrix_get_speed(void) {
    uint8_t speed = 0;
    speed = rgb_matrix_get_speed();
    return speed;
}

void ui_lv_rgb_matrix_set_speed(uint8_t speed, bool eeprom) {
    if (eeprom)
...
uint8_t ui_lv_rgb_matrix_get_mode(void) {
    uint8_t mode = 0;
    mode = rgb_matrix_get_mode();
    return mode;
}

void ui_lv_rgb_matrix_mode(uint8_t mode, bool eeprom) {
    if (eeprom)
...
void ui_lv_tap_keycode(int keycode) {
    switch(keycode){
        case QK_BOOT:
            reset_keyboard();
            break;
...
void ui_lv_set_layer(uint8_t layer) {
    layer_move(layer);
}

uint8_t ui_lv_geT_layer() {
    uint8_t layer = 0;
    layer = get_highest_layer(default_layer_state);
    return layer;
}

uint8_t ui_lv_active_layer(void) {
    uint8_t active_layer = 0;
    active_layer = get_highest_layer(layer_state);
    return active_layer;
}

uint16_t ui_lv_get_cpi(void) {
    uint16_t cpi = 0;
