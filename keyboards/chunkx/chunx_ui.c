...
void ui_task(void) {
    ...
    if (!is_keyboard_left()) {
        ...
        if (hue_redraw || wpm_redraw) {
            ...
            xpos += qp_drawtext_recolor(qp_display, xpos, yrpos, iosevka11, buf, curr_hue, 255, 255, curr_hue, 255, 0);
            if (max_wpm_xpos < xpos) {
                max_wpm_xpos = xpos;
            }
            qp_rect(qp_display, xpos, yrpos, max_wpm_xpos, yrpos + iosevka11->line_height, 0, 0, 0, true);
        ...
        if (hue_redraw || scan_redraw) {
            ...
            qp_rect(qp_display, xpos, yrpos, max_scans_xpos, yrpos + iosevka11->line_height, 0, 0, 0, true);
        }
        ...
        if (hue_redraw || ds_state_redraw) {
            ...
            xpos += qp_drawtext_recolor(qp_display, xpos, yrpos, iosevka11, buf, charybdis_get_pointer_dragscroll_enabled() ? 153 : 255, 255, 255, charybdis_get_pointer_dragscroll_enabled() ? 153 : 255, 255, 0);
            if (max_dss_xpos < xpos) {
                max_dss_xpos = xpos;
            }
            qp_rect(qp_display, xpos, yrpos, max_dss_xpos, yrpos + iosevka11->line_height, 0, 0, 0, true);
        ...
        if (hue_redraw || am_state_redraw) {
            ...
            xpos += qp_drawtext_recolor(qp_display, xpos, yrpos, iosevka11, buf, charybdis_get_auto_mouse_enabled()  ? 34 : 255, 255, 255, charybdis_get_pointer_sniping_enabled() ? 34 : 255, 255, 0);
            if (max_ams_xpos < xpos) {
                max_ams_xpos = xpos;
            }
            qp_rect(qp_display, xpos, yrpos, max_ams_xpos, yrpos + iosevka11->line_height, 0, 0, 0, true);
        ...
        if (hue_redraw || sp_state_redraw) {
            ...
            xpos += qp_drawtext_recolor(qp_display, xpos, yrpos, iosevka11, buf, charybdis_get_pointer_sniping_enabled() ? 153 : 255, 255, 255, charybdis_get_pointer_sniping_enabled() ? 153 : 255, 255, 0);
            if (max_sps_xpos < xpos) {
                max_sps_xpos = xpos;
            }
            qp_rect(qp_display, xpos, yrpos, max_sps_xpos, yrpos + iosevka11->line_height, 0, 0, 0, true);
        ...
        if (hue_redraw || last_keymap_config.raw != keymap_config.raw) {
            ...
            xpos += qp_drawtext_recolor(qp_display, xpos, yrpos, iosevka11, buf, last_keymap_config.nkro? 153 : 255, 255, 255, last_keymap_config.nkro ? 153 : 255, 255, 0);
            //xpos += qp_drawtext_recolor(qp_display, xpos, yrpos, iosevka11, buf1, last_keymap_config.autocorrect_enable? 153 : 255, 255, 255, last_keymap_config.autocorrect_enable ? 153 : 255, 255, 0);
            xpos += qp_drawtext_recolor(qp_display, xpos, yrpos, iosevka11, buf2, last_keymap_config.oneshot_enable? 153 : 255, 255, 255, last_keymap_config.oneshot_enable ? 153 : 255, 255, 0);
            if (max_bpm_xpos < xpos) {
                max_bpm_xpos = xpos;
            }
            qp_rect(qp_display, xpos, yrpos, max_bpm_xpos, yrpos + iosevka11->line_height, 0, 0, 0, true);
    ...
    if (is_keyboard_left()) {
        ...
        if (hue_redraw || rgb_effect_redraw) {
            ...
            xpos += qp_drawtext_recolor(qp_display, xpos, ypos, thintel, buf, curr_hue, 255, 255, curr_hue, 255, 0);
            if (max_rgb_xpos < xpos) {
                max_rgb_xpos = xpos;
            }
            qp_rect(qp_display, xpos, ypos, max_rgb_xpos, ypos + iosevka11->line_height, 0, 0, 0, true);
        }
        ...
        if (hue_redraw || dl_state_redraw) {
            ...
            xpos += qp_drawtext_recolor(qp_display, xpos, ypos, iosevka11, buf, curr_hue, 255, 255, curr_hue, 255, 0);
            if (max_layer_xpos < xpos) {
                max_layer_xpos = xpos;
            }
            qp_rect(qp_display, xpos, ypos, max_layer_xpos, ypos + iosevka11->line_height, 0, 0, 0, true);
        ...
        if (hue_redraw || cpi_redraw) {
            ...
            qp_rect(qp_display, xpos, ypos, max_cpi_xpos, ypos + iosevka11->line_height, 0, 0, 0, true);
        }
        ...
        if (hue_redraw || last_user_state.raw != user_state.raw) {
            ...
            qp_drawtext_recolor(qp_display, 0, ypos, iosevka11, buf, last_user_state.audio_enable? 153 : 255, 255, 255, last_user_state.audio_enable ? 153 : 255, 255, 0);
            xpos += qp_drawtext_recolor(qp_display, xpos, ypos, iosevka11, buf1, last_user_state.audio_clicky_enable ? 153 : 255, 255, 255, last_user_state.audio_clicky_enable ? 153 : 255, 255, 0);
            xpos += qp_drawtext_recolor(qp_display, xpos, ypos, iosevka11, buf2, last_user_state.host_driver_disabled ? 153 : 255, 255, 255, last_user_state.host_driver_disabled ? 153 : 255, 255, 0);
            xpos += qp_drawtext_recolor(qp_display, xpos, ypos, iosevka11, buf3, last_user_state.swap_hands ? 153 : 255, 255, 255, last_user_state.swap_hands ? 153 : 255, 255, 0);
            if (max_upm_xpos < xpos) {
                max_upm_xpos = xpos;
            }
            qp_rect(qp_display, xpos, ypos, max_upm_xpos, ypos + iosevka11->line_height, 0, 0, 0, true);
        }
    }
    qp_flush(qp_display);
