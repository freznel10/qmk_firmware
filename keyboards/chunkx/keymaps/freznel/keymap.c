...
report_mouse_t pointing_device_task_combined_user(report_mouse_t left_report, report_mouse_t right_report)  {
    void tap_tb(uint8_t keycode0, uint8_t keycode1, uint8_t keycode2, uint8_t keycode3) {
        if(abs(left_report.x) + abs(left_report.y) >= cur_factor){
            if(abs(left_report.x) > abs(left_report.y)) {
                if(left_report.x > 0) {
                    for (int8_t i = 0; i <= (abs(left_report.x) + abs(left_report.y)) / cur_factor; i++) {
                        tap_code(keycode0);
                        left_report.x = max(left_report.x - cur_factor, 0);
                    }
                    left_report.y = 0;
                } else {
                    for (int8_t i = 0; i <= (abs(left_report.x) + abs(left_report.y)) / cur_factor; i++) {
                        tap_code(keycode1);
                        left_report.x = min(left_report.x + cur_factor, 0);
                    }
                    left_report.y = 0;
                }
            } else {
                if(left_report.y > 0) {
                    for (int8_t i = 0; i <= (abs(left_report.x) + abs(left_report.y)) / cur_factor; i++) {
                        tap_code(keycode2);
                        left_report.y = max(left_report.y - cur_factor, 0);
                        }
                    left_report.x = 0;
                } else {
                    for (int8_t i = 0; i <= (abs(left_report.x) + abs(left_report.y)) / cur_factor; i++) {
                        tap_code(keycode3);
                        left_report.y = min(left_report.y + cur_factor, 0);
                    }
                    left_report.x = 0;
                }
            }
    ...
    if (scrolling_mode) {
        right_report.h = right_report.x == 0 ? 0 : right_report.x > 0 ? 1 : -1;
        right_report.v = right_report.y == 0 ? 0 : right_report.y > 0 ? 1 : -1;
        right_report.x = 0;
        right_report.y = 0;
    }
    if (is_caret) {
    ...
    if (is_drag_mom) {
        ...
        if(abs(left_report.x) + abs(left_report.y) >= cur_factor) {
            if(abs(left_report.x) > abs(left_report.y)) {
                left_report.h = sign(left_report.x) * (abs(left_report.x) + abs(left_report.y)) / 2;
            } else {
                left_report.v = sign(left_report.y) * (abs(left_report.x) + abs(left_report.y)) / 2; //* (sign(left_report.y)>0 ? 5 : 1) + (sign(left_report.y)==sign(last_v) ? last_v / 8 : 0);
            }
            last_v = left_report.v;
            left_report.x = 0;
            left_report.y = 0;
        } else {
            left_report.v = 0;
   ...
   // if (!g_charybdis_config.is_auto_mouse_enabled) {
//     int8_t x =  left_report.x, y = left_report.y, u = right_report.x, w = right_report.y;
//     left_report.x = 0;
//     left_report.y = 0;
//     right_report.x = 0;
//     right_report.y = 0;
...
void draw_ui_user(void) {
    ...
    if (!is_keyboard_left()) {
        ...
        int  xpos    = 5;
        // if (hue_redraw) {
        //     qp_drawimage_recolor(qp_display, 0, 240 - zerfstudios1->height,  zerfstudios1, curr_hue, 255, 255, curr_hue, 255, 0);
        // }
        ...
        if (hue_redraw || last_led_state.raw != host_keyboard_led_state().raw) {
            ...
            qp_drawimage_recolor(qp_display, 240 - (32 * 3), 0, last_led_state.caps_lock ? lock_caps_on : lock_caps_off, curr_hue, 255, last_led_state.caps_lock ? 255 : 32, curr_hue, 255, 0);
            qp_drawimage_recolor(qp_display, 240 - (32 * 2), 0, last_led_state.num_lock ? lock_num_on : lock_num_off, curr_hue, 255, last_led_state.num_lock ? 255 : 32, curr_hue, 255, 0);
            qp_drawimage_recolor(qp_display, 240 - (32 * 1), 0, last_led_state.scroll_lock ? lock_scrl_on : lock_scrl_off, curr_hue, 255, last_led_state.scroll_lock ? 255 : 32, curr_hue, 255, 0);
        }
    ...
    if (is_keyboard_left()) {
        ...
        if (hue_redraw || last_user_state.raw != user_state.raw) {
            ...
            ypos                    = 240 - (iosevka11->line_height);
            const char* buf = "AUD ";
...
void keyboard_post_init_keymap(void) {
    ...
    // if (!is_keyboard_left()) {
    // qp_animate(qp_display, 240 - matrix->width,  110 - (matrix->height + iosevka11->line_height), matrix);
    // }
